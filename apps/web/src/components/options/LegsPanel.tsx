...
export function LegsPanel({ legs, onRemoveLeg }: LegsPanelProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out">
      <Card
        className={`bg-card border-2 border-border border-t-2 border-x-0 border-b-0 rounded-none transition-all duration-300 ${
          isCollapsed ? 'shadow-lg' : 'shadow-2xl'
        }`}
      >
            ...
            {legs.map((leg) => (
              ...
              >
                <div className="flex items-center gap-4 flex-1 flex-wrap">
                  ...
                  <Badge
                    variant={leg.type === 'call' ? 'outline' : 'secondary'}
                    className="uppercase min-w-[50px] justify-center"
                  >
                    {leg.type}
                  </Badge>
                ...
                <div className="flex items-center gap-4">
                  <div
                    className={`text-sm font-bold ${leg.action === 'buy' ? 'text-red-400' : 'text-green-400'}`}
                  >
