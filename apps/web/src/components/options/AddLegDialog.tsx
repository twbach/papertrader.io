...
export function AddLegDialog({
  ...
  const totalCost =
    (orderType === 'limit' ? parseFloat(limitPrice) || price : price) *
    (parseInt(quantity) || 0) *
    100;
      ...
      <DialogContent className="border-2">
        ...
        <div className="space-y-6 py-4 px-4">
          ...
          <div className="space-y-2">
            ...
            <RadioGroup
              value={orderType}
              onValueChange={(value: string) => setOrderType(value as 'limit' | 'market')}
            >
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="limit" id="limit" />
                <Label htmlFor="limit" className="cursor-pointer text-foreground">
                  Limit Order
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="market" id="market" />
                <Label htmlFor="market" className="cursor-pointer text-foreground">
                  Market Order
                </Label>
              </div>
            </RadioGroup>
          ...
          <div className="p-4 bg-muted rounded border-2 border-border">
            ...
            <div className="text-sm text-muted-foreground mt-1">
              ({quantity} contract{parseInt(quantity) !== 1 ? 's' : ''} × $
              {orderType === 'limit' ? limitPrice : price.toFixed(2)} × 100)
            </div>
          </div>
        </div>
